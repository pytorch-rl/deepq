...
def generate_init_states(path_to_save):
    """
    Generates a validation random initial states and screens and saves it as
    .pickle.
    ...
    file_path = os.path.join(path_to_save, 'validation_set_initial_' + str(
        VALIDATION_SET_SIZE) + '.pickle')
    pickle.dump(init_states, open(file_path, 'wb'))


def generate_random_states(path_to_save):
    """
    Generates a validation set of randomly chosen samples from episodes played
    with randomly initialized policy net and saves it as .pickle.
    ...
    trainer = algorithms.dqn.trainer.DQNTrainer(cfg.TRAIN, env, agent,
                                                target_net,
                                                memory, optimizer,
                                                num_episodes,
                                                device, scheduler,
    ...
    for _ in range(EPISODES_TO_PLAY):
        ...
        episode_states, _ = trainer.agent.play_episode()
        state_list += episode_states

    random.seed(SEED)
    validation_set = random.sample(state_list, k=VALIDATION_SET_SIZE)
    file_path = os.path.join(path_to_save, 'validation_set_random_' + str(
        VALIDATION_SET_SIZE) + '.pickle')
